...
export const registerUser = async (req, res, next) => {
  ...
  try {
    ...
    if (existingUser) {
      return next(errMessage(400, "บัญชีนี้มีผู้ใช้งานแล้ว" ));
    }
    ...
    return next(errMessage(500, "เกิดข้อผิดพลาดระหว่างสร้างผู้ใช้งานใหม่"));
  }
};
...
export const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) return next(errMessage(400, "อีเมลไม่ถูกต้อง"));

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) return next(errMessage(400, "รหัสผ่านไม่ถูกต้อง"));
    ...
    return next(errMessage(500, "เกิดข้อผิดพลาดระหว่างการเข้าสู่ระบบ"));
  }
};
...
export const getUsers = async (req, res, next) => {
  try {
    ...
    return next(errMessage(500, "ดึงข้อมูลจาก users ไม่สำเร็จ"));
  }
};
...
export const getCurrentUser = async (req, res, next) => {
  try {
    ...
    return next(
      errMessage(500, `ดึงข้อมูลผู้ใช้ล้มเหลว ${err.message}`)
    );
  }
};
